...
typedef snd_src_input_data_callback input_callback;
...
static int init_source(int ch, const sndmixer_source_t *srcfns, input_callback callback, void *callback_obj) {
  printf("Init: %d %x %x %x\n", ch, (size_t)srcfns, (size_t)callback, (size_t)callback_obj);
  int chunksz=srcfns->init_source(callback, callback_obj, samplerate, &channel[ch].src_ctx);
...
static void handle_cmd(sndmixer_cmd_t *cmd) {
  if (cmd->cmd==CMD_QUEUE_RAW_S8 || cmd->cmd==CMD_QUEUE_WAV || cmd->cmd==CMD_QUEUE_MOD) {
    int ch=find_free_channel();
    if (ch<0) return; //no free channels
    int r=0;
    printf("Sndmixer: %d: initing source\n", cmd->id);
    if (cmd->cmd==CMD_QUEUE_WAV) {
      r=init_source(ch, &sndmixer_source_wav, cmd->callback, cmd->callback_obj);
    } else if (cmd->cmd==CMD_QUEUE_MOD) {
      r=init_source(ch, &sndmixer_source_mod, cmd->callback, cmd->callback_obj);
    } else if(cmd->cmd==CMD_QUEUE_RAW_S8) {
      printf("Init source\n");
      r=init_source(ch, &sndmixer_source_raw_s8, cmd->callback, cmd->callback_obj);
    }
    if (!r) {
...
int sndmixer_init(int p_no_channels) {
  ...
  if (!channel) return 0;
  curr_id=0;
  cmd_queue=xQueueCreate(SNDMIXER_NUM_IDS, sizeof(sndmixer_cmd_t));
  if (cmd_queue==NULL) {
    free(channel);
    return 0;
  }
...
int sndmixer_queue_raw_s8(input_callback callback, void *callback_obj, int evictable) {
  printf("Stuff: %x %x %d\n", (size_t)callback, (size_t)callback_obj, evictable);
  int id=new_id();
  sndmixer_cmd_t cmd={
                      .id=id,
                      .cmd=CMD_QUEUE_RAW_S8,
                      .callback=callback,
...
int sndmixer_queue_wav(input_callback callback, void *callback_obj, int evictable) {
  int id=new_id();
  sndmixer_cmd_t cmd={
    .id=id,
    .cmd=CMD_QUEUE_WAV,
...
int sndmixer_queue_mod(input_callback callback, void *callback_obj, int evictable) {
  int id=new_id();
  sndmixer_cmd_t cmd={
    .id=id,
    .cmd=CMD_QUEUE_MOD,
