...
extern "C" {
#endif
  /** The maximum number of IDs used by sndmixer */
#define SNDMIXER_NUM_IDS 10

  /**
   * @brief A function pointer used by sound sources to acquire more data.
   * @param obj An object used by the callback. Might contain a file descriptor or a buffer.
   * @param len The maximum number of bytes to read
   * @param buffer The destination buffer
   * @return Number of bytes written
   */
  typedef size_t (*snd_src_input_data_callback)(void *obj, size_t len, uint8_t *buffer);
  ...
  typedef struct {
    ...
    int (*init_source)(const snd_src_input_data_callback callback, void* callback_obj, int req_sample_rate, void **ctx);
    ...
    size_t (*fill_buffer)(void *ctx, int8_t *buffer);
   ...
   *
   * @param no_channels Amount of sounds to be able to be played simultaneously.
   */
  int sndmixer_init(int no_channels);

  int sndmixer_queue_raw_s8(const snd_src_input_data_callback callback, void *callback_obj, int evictable);
  /**
   ...
   *
   * @param callback A callback to get more data
   * @param callback_obj A pointer passed to the callback
   ...
   */
  int sndmixer_queue_wav(const snd_src_input_data_callback callback, void *callback_obj, int evictable);
   ...
   * @return The ID of the queued sound, for use with the other functions.
   */
  int sndmixer_queue_mod(const snd_src_input_data_callback callback, void *callback_obj, int evictable);
